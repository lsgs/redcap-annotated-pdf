...
class AnnotatedPDF extends AbstractExternalModule
{
    ...
    public function redcap_pdf($project_id, $metadata, $data, $instrument, $record, $event_id, $instance) {
        ...
        foreach ($metadata as $fieldIndex => $fieldattr) {
            ...
            if ($fieldattr['element_type']!=='descriptive') {
                ...
                switch ($fieldattr['element_validation_type']) {
                    case 'int':
                        $valtype = 'integer';
                        break;
                    case 'float':
                        ...
                        break;
                    default:
                        $valtype = $fieldattr['element_validation_type'];
                        break;
                }
    ...
    public function redcap_every_page_before_render($project_id) {
            if (empty($project_id) || !(isset($_GET['annotated']) && (bool)$_GET['annotated'])) return;
            global $lang;
            $lang['design_100'] = '{1} '.$lang['design_100']; // Yes
            $lang['design_99' ] = '{0} '.$lang['design_99' ]; // No
            $lang['design_186'] = '{1} '.$lang['design_186']; // True
            $lang['design_187'] = '{0} '.$lang['design_187']; // False
    }
}